...
function App() {
  ...
  useEffect(async () => {
    ...
    const intervalUpdStatus = setInterval(async () => {
        setUpdStatus(new Date());
    }, 1000);
  ...
  const checkIfMoreDataIsNeeded = async (c) => {
    ...
    if (xAxis.max > new Date(sgv.current[0].x).getTime()) {
      ...
      let predDatasetIndex = c.chart.data.datasets.findIndex(x => x.label==='pred')
      if(currBGLinePos.current >= lastDataDate.getTime() ){
        c.chart.setDatasetVisibility(predDatasetIndex, true)
      }else{
        c.chart.setDatasetVisibility(predDatasetIndex, false)
      }
      //predDataset.hidden=null
      ...
      annotation: {
        annotations: [
          
          {
            ...
            backgroundColor: 'rgba(75,255,0,0.05)',//(context) => context.chart.chartArea ? getGradient(context.chart, .8) : null,//'rgba(75,255,0,0.15)',
            borderWidth: 0,
            label: {
              enabled: false,
            }
  ...
  return (
    <div className="App">
      <div className="info" style={{ maxHeight: "30vh" }}>
        ...
        <table className="infoTable">
          <tbody>
            ...
            <tr>
              <td colSpan="2" data-tip="Temporary basal set by your looping software">curr Basal:</td>
              <td data-tip="Temporary basal set by your looping software">{currInfo?.basal}U</td>
              <td data-tip="Basal configured (usually overridden by temp basal)">Basal:</td>
              <td data-tip="Basal configured (usually overridden by temp basal)">{currInfo?.basal}U</td>
            </tr>
            <tr>
              <td></td>
              <td data-tip="CR (or IC) = grams of carbs per unit of insulin">CR:</td>
              <td data-tip="CR (or IC) = grams of carbs per unit of insulin">{currInfo?.reason?.match(/CR: ([^,]*),/)[1]}</td>
              <td data-tip="ISF = 1 unit of insulin will lower your BG by this amount<br />(mmol/l or mg/dl)">ISF:</td>
              <td data-tip="ISF = 1 unit of insulin will lower your BG by this amount<br />(mmol/l or mg/dl)">{currInfo?.reason?.match(/ISF: ([^,]*),/)[1]}</td>
            </tr>
          </tbody>
        </table>
      ...
      {/* [{(Math.abs(updStatus.getTime() - new Date(lastFetch.current)) / 1000).toFixed(0)}s] */}
      [{new Date(updStatus.getTime()-new Date(lastFetch.current).getTime()).getMinutes()}m:{new Date(updStatus.getTime()-new Date(lastFetch.current)).getSeconds()}s]
      Show labels: smb
        <input
          type="checkbox"
          checked={chkSmb}
